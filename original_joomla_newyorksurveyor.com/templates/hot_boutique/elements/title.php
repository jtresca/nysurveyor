...
class JElementTitle extends JElement
{
	...
	function fetchElement( $name, $value, &$node, $control_name )
	{
        ...
        $_show_save 	= ( isset( $node->_attributes['show_save'] ) ) ? $node->_attributes['show_save'] : 0; 
		// the main url
		...
		if ( $_version_file ) {
			$_msg = Jsifr3VersionCheck::getMessage( $_version_file, $_version_path, $_version_url, $_download );
			$_version = Jsifr3VersionCheck::check( $_version_file, $_version_path, $_version_url );
		...
		if ( $_image ) {
			$_image = $_url.'<img src="'.$_image.'" border="0" style="float:right;margin:2px 0 0 10px" alt="" />';
			if ( $_url ) { $_image .= '</a>'; }
		}
		...
		if ( $_show_save ) {
			$_save_button = '<a href="#" onclick="submitbutton(\'save\');" title="'.JText::_( 'Save' ).'"><img align="right" border="0" alt="'.JText::_( 'Save' ).'" src="images/filesave.png"/></a>';
			$html .= $_save_button;
		}
