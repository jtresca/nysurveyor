...
if ( !is_readable ( $install_file ) ) {
	...
	$version = JV15 ? '1.5' : '1.6';
	$mainframe->enqueueMessage( JText::sprintf( 'NNI_NOT_COMPATIBLE', $version ), 'error' );
} else {
...
function installExtension( $name, $title, $type = 'component', $extra = array() )
{
	...
	if ( JV15 ) {
		...
		if ( $type == 'component' ) {
			...
			$file = dirname( __FILE__ ).DS.'files'.DS.'forced'.DS.'administrator'.DS.'components'.DS.'com_'.$name.DS.'com_'.$name.'.xml';
