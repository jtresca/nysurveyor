		...
		switch( $type ) {
			case 'com';
				$sql = "SELECT extension_id FROM #__extensions
					...
					AND `element` = ".$db->quote( 'com_'.$alias )."
					LIMIT 1";
				$db->setQuery( $sql );
				$id = $db->loadResult();
				if ( $id ) {
				...
				break;
			case 'mod';
				$sql = "SELECT id FROM #__modules
					WHERE `module` = ".$db->quote( 'mod_'.$alias )."
					ORDER BY `published` DESC, id LIMIT 1";
				$db->setQuery( $sql );
				$module = $db->loadObject();
				if ( $module ) {
					$ob->link = 'option=com_'.( $has_amm ? 'advanced' : '' ).'modules&view=module&layout=edit&id='.$module->id;
				}
				break;
			case 'plg';
				$sql = "SELECT extension_id, client_id FROM #__extensions
					WHERE `type` = ".$db->quote( 'plugin' )."
					AND `element` = ".$db->quote( $alias )."
					AND `folder` = ".$db->quote( $folder )."
					LIMIT 1";
				$db->setQuery( $sql );
				$id = $db->loadResult();
				if ( $id ) {
					$ob->link = 'option=com_plugins&task=plugin.edit&extension_id='.$id;
				}
				break;
		}
		return $ob;
