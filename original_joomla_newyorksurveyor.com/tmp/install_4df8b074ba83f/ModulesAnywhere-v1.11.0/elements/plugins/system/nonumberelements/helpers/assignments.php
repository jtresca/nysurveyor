...
class NoNumberElementsAssignmentsHelper
{
	...
	{
		if ( 	!$this->_params->id
			||	!( 		( $this->_params->option == 'com_content' && $this->_params->view == 'article' )
					||	( $this->_params->option == 'com_flexicontent' && $this->_params->view == 'items' )
				)
		) {
			return ( $assignment == 'exclude' );
		}
	 ...
	 * @return <bool>
	 */
	function passTags_K2( &$params, $selection = array(), $assignment = 'all' )
	{
		if ( $this->_params->option != 'com_k2' ) {
			return ( $assignment == 'exclude' );
		}
		...
		$pass = (
				( $params->inc_tags && JRequest::getCmd( 'tag' ) )
			||	( $params->inc_items && $this->_params->view == 'item' )
		);
		...
		if ( $params->inc_tags && JRequest::getCmd( 'tag' ) ) {
			...
			$query = 'SELECT t.name'
				...
				.' ON t.id = x.tagID'
				.' WHERE x.itemID = '.(int) $this->_params->id
				.' AND t.published = 1'
				;
			$this->_db->setQuery( $query );
		...
		return $this->passSimple( $tags, $selection, $assignment );
	}
	 ...
	 * @return <bool>
	 */
	function passItems_K2( &$params, $selection = array(), $assignment = 'all' )
	{
		if ( !$this->_params->id || $this->_params->option != 'com_k2' || $this->_params->view != 'item' ) {
			return ( $assignment == 'exclude' );
		}

		$selection = $this->makeArray( $selection );

		return $this->passSimple( array( $this->_params->id ), $selection, $assignment );
	}
	...
	{
		$month = $this->_date->toFormat( '%m' ); // 01 (for January) through 12 (for December)
	...
	{
		$day = $this->_date->toFormat( '%u' ); // 1 (for Monday) though 7 (for Sunday)
