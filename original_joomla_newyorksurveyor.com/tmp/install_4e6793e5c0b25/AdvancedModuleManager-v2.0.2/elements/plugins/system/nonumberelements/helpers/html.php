...
class nnHTML
{
	...
	function selectlist( &$options, $name, $value, $id, $size = 0, $multiple = 0, $attribs = '', $j15 = 0 )
	{
		...
		if ( $multiple ) {
			$document =& JFactory::getDocument();
			$document->addScript( JURI::root(true).'/plugins/system/nonumberelements/fields/multiselect/script.js?v='.$this->_version );
			$document->addStyleSheet( JURI::root(true).'/plugins/system/nonumberelements/fields/multiselect/style.css?v='.$this->_version );
