...
if ( typeof( nn_scripts_version ) == 'undefined' || nnScriptsVersionIsNewer( nn_scripts_version, '2.5.0' ) ) {
	// version number of the script
	// to prevent this from overwriting newer versions if other extensions include the script too
	var nn_scripts_version = '2.5.0';
...
window.addEvent( 'domready', function() {
	...
	if ( document.getElement( '#nn_param_preloader' ) ) {
		document.getElement( '#nn_param_preloader' ).setStyles({
			visibility: 'hidden'
		});
		( function() {
			document.getElement( '#nn_param_preloader_container' ).innerHTML = '';
			document.getElement( '#nn_param_preloader' ).setStyles({
				visibility: 'visible'
			}).injectInside( document.getElement( '#nn_param_preloader_container' ) );
		} ).delay( 2000 );
	}
});
